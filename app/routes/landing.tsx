...
export default function LandingPage() {
	...
	return (
		<div className="relative flex min-h-screen flex-col overflow-hidden bg-background text-foreground">
			...
			<header className="relative border-b border-border/50 bg-background/60 backdrop-blur-xl">
				<div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between px-6 md:px-10">
					...
					<nav className="hidden items-center gap-10 text-sm text-muted-foreground md:flex">
						<span className="cursor-default transition-colors hover:text-foreground">Showcase</span>
			...
			<main className="relative mx-auto flex w-full max-w-7xl flex-1 items-start justify-center px-6 pb-24 pt-24 md:px-10 md:pt-28">
				<section className="relative isolate flex w-full max-w-4xl flex-col items-center text-center">
					<div className="pointer-events-none absolute -left-28 top-10 -z-10 hidden w-60 rounded-3xl border border-border/70 bg-card/45 p-4 backdrop-blur-xl lg:block">
					...
					<div className="pointer-events-none absolute -right-36 top-3/5 -z-10 hidden w-52 rounded-3xl border border-border/70 bg-card/45 p-4 backdrop-blur-xl lg:block">
					...
					<div className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/15 px-4 py-1 text-xs font-semibold uppercase tracking-widest text-primary">
						<span className="h-2 w-2 rounded-full bg-primary" />
						v2.0 now available
					</div>

					<h1 className="mt-7 text-balance text-4xl font-bold leading-[1.05] tracking-tight sm:text-5xl md:text-7xl">
					...
					<p className="mt-6 max-w-3xl text-pretty text-lg leading-8 text-muted-foreground">
						Describe what you need and ASB shapes a polished landing page in seconds. No code, no
						drag-and-drop, just a cleaner way to launch.
					</p>

					<div className="mt-12 w-full rounded-[2rem] border border-border/70 bg-card/60 p-3 shadow-2xl shadow-primary/15 backdrop-blur-xl">
						<div className="flex flex-col gap-3 rounded-[1.6rem] bg-background/70 p-3 sm:flex-row sm:items-center sm:gap-4 sm:p-4">
							<div className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl px-2 py-1 text-left text-base text-muted-foreground sm:px-4">
								...
								<span className="line-clamp-2 sm:line-clamp-1">
									Describe your website (e.g. a SaaS pricing page for startup teams)...
								</span>
							</div>
							<button
								type="button"
					...
					<div className="mt-7 flex w-full max-w-4xl flex-wrap items-center justify-center gap-2 text-sm text-muted-foreground">
						<span className="mr-1 font-medium text-muted-foreground">Try asking for:</span>
			...
			<footer className="relative border-t border-border/50 bg-background/55 backdrop-blur-xl">
				<div className="mx-auto flex w-full max-w-7xl flex-col items-center justify-between gap-4 px-6 py-7 md:flex-row md:px-10">
					<p className="text-sm text-muted-foreground">Trusted by 10,000+ creators building the future.</p>
