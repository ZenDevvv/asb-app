	...
	return (
		<div className="relative flex min-h-screen flex-col overflow-hidden bg-background text-foreground">
			...
			<main className="relative mx-auto flex w-full max-w-7xl flex-1 items-start justify-center px-6 pb-24 pt-24 md:px-10 md:pt-28">
				...
				>
					<motion.div
						...
						animate={prefersReducedMotion ? { opacity: 1, x: 0, y: 0 } : { opacity: 1, x: 0, y: [0, -10, 0] }}
						transition={
							prefersReducedMotion
					...
					<motion.div
						className="pointer-events-none absolute -right-36 top-3/5 -z-10 hidden w-52 rounded-3xl border border-border/70 bg-card/45 p-4 backdrop-blur-xl lg:block"
						initial={{ opacity: 0, x: prefersReducedMotion ? 0 : 20, y: prefersReducedMotion ? 0 : 10 }}
						animate={prefersReducedMotion ? { opacity: 1, x: 0, y: 0 } : { opacity: 1, x: 0, y: [0, 8, 0] }}
						transition={
							prefersReducedMotion
					...
					<motion.p variants={revealItem} className="mt-6 max-w-3xl text-pretty text-lg leading-8 text-muted-foreground">
			...
			<motion.footer
				...
				animate={{ opacity: 1, y: 0 }}
				transition={{ duration: prefersReducedMotion ? 0.2 : 0.45, ease: "easeOut", delay: 0.3 }}
			>
