...
export function CardBlock({ block, globalStyle, isEditing }: BlockComponentProps) {
	const { title, text, buttonText, buttonUrl, imageSrc, imageAlt } = block.props as {
		title?: string;
		text?: string;
		buttonText?: string;
		buttonUrl?: string;
			...
			<article
				...
				<div className={`space-y-3 ${textAlignClass}`}>
					...
					<p className={`leading-relaxed ${bodySizeClass}`}>
						{text ||
							"Use cards to group related content and CTAs in a clean, scannable layout."}
					</p>
