...
function getAppearanceConfig(
	...
	switch (appearance) {
		...
		case "link":
			return {
				className:
					"cursor-pointer inline-flex items-center gap-1.5 font-semibold transition-opacity hover:opacity-70 underline underline-offset-4",
				style: { color: accentColor },
			};
		case "text":
			return {
				className:
					"cursor-pointer inline-flex items-center gap-1.5 transition-opacity hover:opacity-70",
				style: { color: accentColor },
			};
		case "solid":
		default:
...
export function ButtonBlock({ block, globalStyle, isEditing }: BlockComponentProps) {
