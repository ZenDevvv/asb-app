...
export function GroupModeSettings({
	...
	const colCount: 1 | 2 | 3 = currentSpansLen === 1 ? 1 : currentSpansLen === 2 ? 2 : 3;
						...
						.map((block) => {
							...
							return (
								<button
									key={block.id}
									onClick={() =>
										selectBlock(section.id, activeGroup.id, block.id)
									}
									className={cn(
						...
						<div className="grid grid-cols-4 gap-1">
							{(
								...
								<button
									key={label}
									onClick={() =>
										updateGroupStyle(section.id, activeGroup.id, { gap: value })
									}
									className={cn(
	...
	return (
		<div className="space-y-3">
			...
			<div className="space-y-1.5">
				...
				<Select
					value={activePreset}
					onValueChange={(v) => handlePresetChange(v as ColumnPreset)}>
