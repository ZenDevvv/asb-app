...
type BlockType =
  ...
  | "image"          // Single image — editableProps: src, alt, caption (short-text). editableStyles are grouped in Block Mode: Style panel -> width, borderRadius, opacity, height, tilt (-180 to +180, default 0); Caption panel (collapsed by default) -> fontSize (with custom), fontWeight, fontStyle, letterSpacing, textAlign (align-picker), captionVerticalAlign (top/center/bottom), captionPadding. Caption is full-width, absolutely positioned; horizontal alignment via textAlign, vertical via captionVerticalAlign. Supports block-level font family override applied to caption (shown in Caption panel). supportsCustomTextSize = true (same as heading/text).
...
- `image` blocks split style controls into **Style** and **Caption** collapsibles. **Style** contains Width/Corner Style/Opacity/Height/Tilt and does not show Font Family. **Caption** is collapsed by default and contains Font Family + caption typography/alignment/padding controls.
